 ...
 */
export const getProductoConImagenes = async (productoId: number) => {
 ...
 */
export const listProductosConImagenes = async (where?: any) => {
  const productos = await prisma.producto.findMany({ where })
  return productos.map(p => ({
    ...p,
    imagenesArray: parseImagenesJSON((p as any).imagenes),
  }))
}
